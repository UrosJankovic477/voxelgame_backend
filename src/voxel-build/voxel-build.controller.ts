import { BadRequestException, Bind, Body, Controller, Delete, Get, Param, ParseUUIDPipe, Post, Query, Req, Request, UploadedFile, UploadedFiles, UseGuards, UseInterceptors } from "@nestjs/common";
...
export class VoxelBuildController {
    constructor(
        private voxelBuildService: VoxelBuildService, private commentService: CommentService,
    ...
    public async createComment(@User() user, @Param('uuid', ParseUUIDPipe) uuid: UUID, @Body() body: {content: string}) {
        return await this.commentService.createComment(body.content, user.username, uuid);
